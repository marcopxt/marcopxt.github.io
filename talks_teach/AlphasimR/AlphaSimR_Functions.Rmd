...
As we recall from the first vignette, in a way to implement a set of simulations in the `AlphaSimR` four steps must be followed:
...
First, at the beginning of each breeding cycle, we generate new crosses.
...
There are three options for assuming the error variance mentioned. We can set the argument *h2*, *H2*, *varE*. The user should use only one of these arguments. If the user supplies values for more than one, only one will be used according to the order in which they are listed above.
