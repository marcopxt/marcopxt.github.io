...
First implementation will use one additive trait. Let's create the founder genome and also a base population and assume the trait characteristics. In addition to the argument that we have been describing, we need to use the function `addSnpChip` to add the SNPs for each individual.
...
Now that we have a calibration, we can deploy genomic selection by using the marker effects to prediction, together with the genotypes of the target population, the estimated breeding value of the individuals.
...
So, we can implement the same pipeline before deployed, but account for dominance effects as well. In this case, the may use the function `RRBLUP_D` from *AlphaSimR*.
...
Measuring the accuracies to see how was the impact of the dominance effect into the model.
...
#Evaluate accuracy
cor(gv(GCA_Male), ebv(GCA_Male))

cor(gv(GCA_Female), ebv(GCA_Female))
...
# Split the founders into two population (distinct groups)
lines = newPop(founderPop[1:100])
tester = newPop(founderPop[101:102])
...
#Evaluate accuracy
cor(gv(SCA_pop), ebv(SCA_pop))


```


## Fixed effects into the population

We can also use fixed effect into the model. The fixed effects get treated as a categorical variable and are used to construct a sum-to-zero design matrix. When we have years and locations, you want a unique fixed effect for each year by location combination. 
...
For such, we will create a base genome and implement two traits for latter prediction via genomic selection.
...
We can use the information from the individuals to fit the model in BGLR. So, the first step will be to get that information from the populations. We may get the phenotypic values for the individuals, the SNP data and create a relationship Matrix out of it.
...
Same implementation could be done with the information of more than one trait at a time. So, we will do the same implementation before mentioned, but using two traits, as follows:
